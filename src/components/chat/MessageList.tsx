                    ...
                    <>
                      {message.parts.map((part, partIndex) => {
                        switch (part.type) {
                          ...
                          case "reasoning":
                            return (
                              <div key={partIndex} className="mt-3 p-3 bg-purple-100 rounded-md border border-purple-300">
                                <span className="text-xs font-medium text-purple-900 block mb-1">Reasoning</span>
                                <span className="text-sm text-purple-950">{part.reasoning}</span>
                              </div>
                            );
                          case "tool-invocation":
                            const tool = part.toolInvocation;
                            return (
                              <div key={partIndex} className="inline-flex items-center gap-2 mt-2 px-3 py-1.5 bg-purple-100 rounded-lg text-xs font-mono border border-purple-300">
                                {tool.state === "result" && tool.result ? (
                                  <>
                                    <div className="w-2 h-2 rounded-full bg-emerald-500"></div>
                                    <span className="text-purple-950">{tool.toolName}</span>
                                  </>
                                ) : (
                                  <>
                      ...
                      {isLoading &&
                        ...
                        messages.indexOf(message) === messages.length - 1 && (
                          <div className="flex items-center gap-2 mt-3 text-purple-100">
